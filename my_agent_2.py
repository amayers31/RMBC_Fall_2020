...
class Ayers(Player):
	...
	def choose_move(self, possible_moves, seconds_left):
		...
		if self.pawn_q:
			pq = self.pawn_q
			self.pawn_q = None
			if not self.board.piece_at(pq) is None and self.board.piece_at(pq).piece_type == chess.PAWN and self.board.piece_at(pq).color == self.color:
				if self.color:
					i = 1
				else:
					i=-1
				if not self.board.piece_at(pq+i*7) is None and not self.board.piece_at(pq+i*7).color == self.color:
					return chess.Move(pq, pq+i*7, chess.QUEEN)
				if not self.board.piece_at(pq+i*9) is None and not self.board.piece_at(pq+i*9).color == self.color:
